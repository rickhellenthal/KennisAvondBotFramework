...
- Voeg de onderstaande functie inclusief implementatie toe aan `UserProfileDialog.cs`.
...
- Vervang de functionaliteit in `OnMessageActivityAsync()` zoals gemaakt in opdracht 2.4 met het volgende:
    ...
    - Start, indien de naam van de gebruiker nog niet bekend is, het zojuist aangemaakte dialoog.
    - Indien de naam van de gebruiker wel bekend is, de bot reageert met iets als `'Hoi {naam}'`.

- Start, indien er geen ander actief dialoog is en de naam van de gebruiker nog niet bekend is, het zojuist aangemaakt dialoog.
Gebruik als dialogId `nameof(UserProfileDialog)`.
...
- Dit dialoog als eerste stap aan de gebruiker de vraag stelt `'Wat is je vraag {gebruikersnaam}?'`.
- Dit dialoog als tweede stap reageert met `'Sorry, ik heb geen antwoord op de vraag {vraag}.'`.
